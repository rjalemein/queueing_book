...
\begin{exercise}[Queue with Blocking] A queueing system
...
\begin{exercise}[Estimating the lead time distribution] 
\label{ex:19} Take $d_k = \min\{L_{k-1}+a_k, c_k\}$, and assume that jobs are served in FIFO sequence.
  ...
  \begin{hint}
  Consider a numerical example. Suppose $L_{k-1}=20$. Suppose
  ...
  \end{hint}
    \begin{solution}
      ...
      tagged customer sees $L_{k-1}$ customers in the system, hence the
      tagged customer's service can only start after all $L_{k-1}$
      customers have been served (assuming FIFO scheduling of course).  Now, if $L_{k-1}-c_k \geq 0$, there are
      still people in front of the tagged customer, either in queue or in service. In fact, as long
      as $c_k+c_{k+1}+\cdots +c_m \leq L_{k-1}$, there are still customers in
      front of the tagged customer. Therefore, when the tagged customer leaves the system, the inequality $c_k+c_{k+1}+\cdots +c_m > L_{k-1}$ must hold.

      We can also tag the last customer that arrives in period
      $k$. This customer will certainly have left if $m$ is such that
      $c_k+\cdots+c_m \geq L_{k-1}+a_k$.
...
\begin{extra}[Rework] \label{ex:52}
...
\begin{extra}[Queue length dependent server capacity]\label{ex:50}
...
\begin{extra}[Fair queueing] \label{ex:51}
...
and likewise for the other queue.
  ...
  \begin{exercise} [Priority queueing] An interesting situation is a system with two queues served by one server, but such that one queue, queue $A$, gets priority over the other queue.
...
\begin{exercise} [Queue with protected service capacity and lost capacity] Consider a
  ...
  Let $r_A$ be the reserved capacity for queue A, and likewise for
   ...
   \begin{solution} Lueue A can use all capacity, except what is
...
\begin{extra}[\faRocket\/ Queue with setups]
